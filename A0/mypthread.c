...
static void schedule(int signum) {
    if (__sync_add_and_fetch(&mode_bit, 1) == 1) {
        ...
        if (signum == MUTEX_LOCKED) {
            //printf("Your ran too long %u \n", MTH->current->tcb->tid);
            enqueue(MTH->current, MTH->mutex_locked);
            // printQueue(MTH->ready);
        }
//        If a mutex is unlocked then find the
        if (signum == MUTEX_UNLOCKED) {
            tcb_queue *tempQ = createQueue("resource");
            tcb_node *node;
            while(peek(MTH->mutex_locked)){
                node = dequeue(MTH->mutex_locked);
                if(!isOnMutexHold(node->tcb)){
...
// Foresight:Does locking mutex by removing from wait queue here safe?
            ...
            free(firstWaitingThreadNode);
                if (__sync_add_and_fetch(&mode_bit, -1) == 0) {
                    schedule(MUTEX_LOCKED);
                }
        }

        if (mutex->guard == 1 && mutex->owner->tid == MTH->current->tcb->tid) {
            printf("Thread tried to lock a mutex that is locked by itself");
            exit(1);
        }
...
//    return 0;
    if (__sync_add_and_fetch(&mode_bit, 1) == 1) {
        ...
        if (mutex->guard == 1 && mutex->owner->tid == MTH->current->tcb->tid) {
            ...
            mutex->guard = 0;
            if (__sync_add_and_fetch(&mode_bit, -1) == 0) {
                schedule(MUTEX_UNLOCKED);
            }

            return 1;
        }
//        if (peek(mutex->hold_queue) != NULL) {
//            tcb_node *nextWaitingThreadNode = dequeue(mutex->hold_queue);
//            mutex->guard = 1;
//            mutex->owner = nextWaitingThreadNode->tcb;
//            free(nextWaitingThreadNode);
//            if (__sync_add_and_fetch(&mode_bit, -1) == 0) {
//                schedule(MUTEX_UNLOCKED);
//            }
//            return 1;
//        }
        printf("Thread tried to unlock a mutex that is already free");
        exit(1);
    }
}
...
int mypthread_mutex_destroy(mypthread_mutex_t *mutex) {
    if (__sync_add_and_fetch(&mode_bit, 1) == 1) {
        disableTimer();
        if (mutex->guard == 1 || peek(mutex->hold_queue) != NULL) {
            printf("Uh Um, Tried to destroy a mutex that is being used by other threads.");
            exit(1);
        }
